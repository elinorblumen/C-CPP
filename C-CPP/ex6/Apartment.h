...
class Apartment {
 ...
 public:
  /**
   * Constructor that get pair of points, and
   * creates new apartment.
   * @param Coordinates std::pair, which the first
   * is the x coordinate and the second is the y.
   */
  Apartment (const std::pair <double, double>& Coordinates);
  Apartment (std::initializer_list<double>& in);
  /**
   *
  ...
  double get_y () const;
  /**
   * Operator <, apartment is smaller than other
   * if it closer to [31.772425, 35.213506]
   * @param other
   * @return true, if this apartment is smaller
   * than the other one, false otherwise
   */
  bool operator< (const Apartment &other) const;
  /**
   * Operator >, apartment is greater than other
   * if it farther from [31.772425, 35.213506]
   * @param other
   * @return true, if this apartment is greater
   * than the other one, false otherwise
   */
  bool operator> (const Apartment &other) const;
  /**
   * Operator ==, Two apartments are identical if
   * their x and y values are the same,
   * to the point of an epsilon = 0.0001
   * @param other
